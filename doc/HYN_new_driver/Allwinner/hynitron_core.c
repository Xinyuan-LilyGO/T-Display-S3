...
static int hyn_irq_registration(struct i2c_client *client)
{
  //  struct device_node *node = NULL;
	...
	ret = input_request_int(&(config_info.input_type), hyn_eint_interrupt_handler,
				 hyn_ts_data->pdata->irq_gpio_flags, hyn_ts_data);
	if (ret) {
		HYN_ERROR("request irq failed\n");
	}else {
		HYN_INFO("IRQ request succussfully, irq=%d ,trigger_rising:%d .", config_info.irq_gpio.gpio,HYN_IRQ_TRIGGER_RISING_CONFIG);
...
static int __init hynitron_driver_init(void)
{
	...
	if (config_info.ctp_used == 0) {
		HYN_ERROR("if use ctp,please put the sys_config. ctp_used set to 1.\n");
		return 0;
	}	
